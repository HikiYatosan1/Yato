...
import { ArrowRight, Cable, House, Tv, Wifi } from "lucide-react";
...
export function TariffCard({ tariff }: TariffCardProps) {
  ...
  const Icon = tariffIcons[tariff.category];
    ...
    <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.26 }} className="h-full">
      <Card
        className={cn(
          ...
          tariff.popular && "border-avanta-green/28 shadow-float",
        )}
      >
        ...
        <div className="relative z-10 flex items-start justify-between gap-3">
          ...
          {tariff.badge ? <Badge>{tariff.badge}</Badge> : null}
        </div>
        ...
        <div className="relative z-10 mt-auto pt-8">
          ...
          >
            <Link to={to(`/?action=connect&tariff=${encodeURIComponent(tariff.name)}#application`)}>
              {mode === "site" ? "Подключить" : "Подключить demo"}
